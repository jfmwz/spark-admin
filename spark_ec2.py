...
def wait_for_instances(conn, instances):
  while True:
    ...
    if len([i for i in instances if i.state == 'pending']) > 0:
	  ...
	  if len([i for i in instances if utils.isOpen(i.public_dns_name, 22) == False]) > 0:
	    time.sleep(5)
	  else:
	  	return
...
def launch_cluster(conn, opts, cluster_name):
  ...
  if master_group.rules == []: # Group was just now created
    ...
    master_group.authorize('tcp', 10000, 10000, '0.0.0.0/0')
...
def set_hostname(cluster_name, opts, master_nodes, slave_nodes):
  master = master_nodes[0].ip_address + " " + cluster_name +"-master\n"
  slave = ""
  for i in range(len(slave_nodes)):
      slave = slave + slave_nodes[i].ip_address + " " + cluster_name + "-slave" + str(i) + "\n"
  fo = open(cluster_name, "wb")
  fo.write(master)
  fo.write(slave)
  fo.close()
  scp(master_nodes[0].public_dns_name, opts, cluster_name, "/etc/hosts")
  for node in slave_nodes:
      scp(node.public_dns_name, opts, cluster_name, "/etc/hosts")
  os.remove(cluster_name)
...
# def ssh_new(host, opts, command):
#   client = paramiko.SSHClient()
#   client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
#   client.connect(host, username=opts.user, key_filename=opts.identity_file)
...
def main():
  ...
  if action == "launch":
    ...
    set_hostname(cluster_name, opts, master_nodes, slave_nodes)
